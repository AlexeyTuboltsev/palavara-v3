...
const DEFAULT_OPTIONS: Required<TOptimizeOptions> = {
  jpegQuality: 85,
  webpQuality: 80,
  avifQuality: 65,
  lqipQuality: 30,
 ...
 * @param outputDir - Base output directory (e.g., './public/img')
 * @param options - Optimization options
 * @returns Manifest entry with all generated files and metadata
 */
async function optimizeImage(
  inputPath: string,
  outputDir: string,
  options?: TOptimizeOptions
): Promise<TOptimizedImageResult> {
  // Merge options with defaults
  const opts: Required<TOptimizeOptions> = {
    ...DEFAULT_OPTIONS,
    ...options
  };
 ...
 */
function calculateSavings(original: number, optimized: number): number {
