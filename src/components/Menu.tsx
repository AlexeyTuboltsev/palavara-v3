...
export const Menu: FC<{ state: TReadyAppState, type: 'blue' | 'yellow' }> = ({state, type}) => <div
  ...
  {(state.menu.root as TRootMenuItem).children.map((menuItemId: string) =>
    <MenuItem key={menuItemId} menu={state.menu} menuItemId={menuItemId} type={type}/>)
  }
  {state.screenSize === EScreenSize.MOBILE && state.menuIsCollapsible &&
      <MenuButton/>
  }
  {(state as any).screenSize === EScreenSize.MOBILE &&
      <SectionMenuSecondary sectionMenu={state.sectionMenu} type={type}/>
  }
</div>


export const MenuSec: FC<{ state: TReadyAppState }> = ({state}) =>
  <Menu state={state} type={'blue'}/>

export const MenuHome: FC<{ state: TReadyAppState }> = ({state}) =>  <div
    className={cn(styles.menu, styles.menuBlue, styles.transparency, {[styles.menuHidden]: !(state as any).menuIsOpen})}>
    {(state.menu.root as TRootMenuItem).children.map((menuItemId: string) =>
